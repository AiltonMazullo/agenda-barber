...
export default function FormasPagamentoDespesaPage() {
  const { barbershop } = useAuth();
  const { methods, isLoading, remove } = useExpensePaymentMethods(barbershop?.id);
  const [toDelete, setToDelete] = useState<ExpensePaymentMethod | null>(null);

  return (
    <div className="space-y-5 p-4 md:p-6 bg-surface-base min-h-screen text-foreground">
      <PageHeader
        title="Formas de pagamento de despesa"
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/financial/formas-pagamento-despesa/novo"
              ...
              <div className="min-w-0">
                ...
                {m.autoMarkAsPaid && (
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Marcado como pago automaticamente
                  </p>
                )}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <Link
                  href={`/financial/formas-pagamento-despesa/${m.id}`}
                  title="Editar"
                ...
                <Link
                  href={`/financial/formas-pagamento-despesa/${m.id}?mode=view`}
                  title="Visualizar"

...
import { clientClubBannerService } from "@/services/client-club-banner.service";

interface ClubBannerCarouselProps {
  barbershopId: string;
  /** Exibido enquanto carrega ou quando não há banners do clube cadastrados. */
  fallback?: ReactNode;
}
...
/** Banners do clube/assinatura — substitui o `BarbershopHero` genérico na área logada (§2.3). */
export function ClubBannerCarousel({ barbershopId, fallback = null }: ClubBannerCarouselProps) {
  const [banners, setBanners] = useState<HeroBanner[]>([]);
  const [index, setIndex] = useState(0);

  useEffect(() => {
    let active = true;
    clientClubBannerService
